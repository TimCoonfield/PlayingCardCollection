...
export interface CreatorEditorValues {
  name: string;
...
export function CreatorEditor({
  ...
  function closeEditor() {
    if (heroImageUrl && heroImageUrl !== persistedHeroUrl) requestBlobCleanup(heroImageUrl);
    ...
    >
      ...
      >
        <div className="flex items-center justify-between gap-3">
          <h2 className="font-display text-lg font-semibold text-felt-ink">Edit Creator</h2>
          <button
            type="button"
            onClick={closeEditor}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="Canonical credit name" error={state.fieldErrors?.name}>
            <input name="name" required maxLength={200} defaultValue={values.name} className={inputClass} />
          </Field>
          <Field label="Public display name" error={state.fieldErrors?.displayName}>
            <input name="displayName" maxLength={240} defaultValue={values.displayName ?? ""} className={inputClass} />
          </Field>
        </div>

        <Field label="Tagline" error={state.fieldErrors?.tagline}>
          <input name="tagline" maxLength={300} defaultValue={values.tagline ?? ""} className={inputClass} />
        </Field>
        ...
        <Field label="About this Creator (Markdown)" error={state.fieldErrors?.description}>
          <textarea name="description" rows={10} defaultValue={values.description ?? ""} className={inputClass} />
        </Field>

        <label className="flex items-start gap-3 rounded-md border border-felt-line bg-felt-bg/50 p-3">
          <input type="checkbox" name="favorite" defaultChecked={values.favorite} className="mt-1 accent-brass" />
          <span>
            <span className="block text-sm font-medium text-felt-ink">Favorite Creator</span>
            <span className="block text-xs leading-5 text-felt-sub">
              Favorite Creators appear on the homepage and receive an image-backed directory tile.
            </span>
          </span>
        </label>

        <p className="text-xs text-felt-sub/70">The Creator URL stays unchanged when the name changes.</p>
        <button
          type="submit"
        ...
        >
          {heroUploading ? "Uploading image…" : pending ? "Saving…" : "Save Creator"}
        </button>
      </form>
    </div>
  );
