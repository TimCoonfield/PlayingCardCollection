...
import { CoinFilters } from "@/components/coin-filters";
...
export default async function CoinsPage({
  ...
  const and: Prisma.CoinWhereInput[] = [];
  ...
  const [coins, total, designers, producers, seriesList, session] = await Promise.all([
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <h1 className="font-display text-xl font-semibold text-felt-ink">
          Coins <span className="font-sans text-base font-normal text-felt-sub">({total})</span>
        </h1>
        {isAuthenticated && (
          <Link
            href="/coins/new"
      ...
      {coins.length === 0 ? (
        <p className="py-16 text-center text-felt-sub">No coins match these filters.</p>
      ) : (
      ...
      <Pagination page={page} totalPages={totalPages} searchParams={currentSearchParams} basePath="/coins" />
    </div>
  );
}
