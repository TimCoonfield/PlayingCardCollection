...
Search is case-insensitive. A Deck can expose multiple normalized names in its \`designers\` array; the singular \`designer\` response field is retained as a joined compatibility credit. Repeated \`designer\`, \`producer\`, or \`series\` parameters match any listed value; repeated \`tag\` parameters require all listed tags. Supported scopes are \`all\`, \`name\`, \`creator\`, \`series\`, \`producer\`, and \`notes\`. Supported sorts are \`relevance\`, \`name-asc\`, \`name-desc\`, \`year-asc\`, \`year-desc\`, and \`recent\`. Optional filters are \`minYear\`, \`maxYear\`, \`favorite\`, \`whiteWhale\`, and \`hasPhoto\`. Pagination uses \`limit\` (1-100, default 50) and \`offset\`; follow \`pagination.nextUrl\` until it is null for collection-wide analysis.
